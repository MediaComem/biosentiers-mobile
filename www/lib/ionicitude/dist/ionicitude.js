...
function Ionicitude($log, $q) {
  ...
  function listLibActions() {
    $log.log(TAG + "lib actions", Object.getOwnPropertyNames(lib));
  ...
  function checkDevice() {
    $log.log(TAG + 'checking device');
    var q = $q.defer();
    plugin.isDeviceSupported(function (success) {
      $log.log(TAG + "checkDevice success", success);
      service.deviceSupportsFeatures = true;
      q.resolve(success);
    }, function (error) {
      $log.log(TAG + "checkDevice error", error);
  ...
  function init(params) {
    return $q(function (resolve, reject) {
      if (!initialized) {
        $log.log(TAG + 'init service starting');
        loadPlugin()
          .then(setWrappers)
          .then(setCallback)
          .then(function () {
            if (doDeviceCheck()) {
              ...
              return $log.log(TAG + 'check skipped due to init settings');
            }
          })
          .then(handleSettings)
          .then(function () {
          ...
          .then(handleQueue)
          .then(function () {
            $log.log(TAG + 'end of initialization');
            resolve(service);
          })
  ...
  function executeActionCall(call) {
    ...
    $log.log(TAG + "executing action call", call, action);
